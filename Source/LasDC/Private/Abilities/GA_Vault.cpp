...
/** Actually activate ability, do not call this directly */
 void UGA_Vault::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo,
	const FGameplayEventData* TriggerEventData) 
{
	 ...
	 ACharacterBaseGAS* Character = GetCharacterBaseFromActorInfo();
	 Character->CameraLockedDelegate.Broadcast(true);
	 /* Movement */
	 ...
	 /* Motion Warping*/
	 UMotionWarpingComponent* MotionWarpingComponent = Character ? Character->GetMotionWarpingComponent() : nullptr;
	 ...
	 MontageTask->OnBlendOut.AddDynamic(this, &UGA_Vault::K2_EndAbility); //deleagte kutsutaan k2endabiltiy kun out montage
	 MontageTask->OnCompleted.AddDynamic(this, &UGA_Vault::K2_EndAbility); //deleagte kutsutaan k2endabiltiy kun out montage
	 MontageTask->OnInterrupted.AddDynamic(this, &UGA_Vault::K2_EndAbility); //deleagte kutsutaan k2endabiltiy kun out montage
	 MontageTask->OnCancelled.AddDynamic(this, &UGA_Vault::K2_EndAbility); //deleagte kutsutaan k2endabiltiy kun out montage
	 MontageTask->ReadyForActivation(); //IMPORTANT
}
...
/** Native function, called if an ability ends normally or abnormally. If bReplicate is set to true, try to replicate the ending to the client/server */
 void UGA_Vault::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo,
	bool bReplicateEndAbility, bool bWasCancelled)
 {
	 ...
	 /* Motion Warping */
	 UMotionWarpingComponentBase* MotionWarpingComponent = Character ? Character->GetMotionWarpingComponent() : nullptr;
	 if (MotionWarpingComponent) {
	 ...
	 Character->CameraLockedDelegate.Broadcast(false);
	 Super::EndAbility(Handle, ActorInfo, ActivationInfo, bReplicateEndAbility, bWasCancelled);

}

 bool UGA_Vault::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags,
